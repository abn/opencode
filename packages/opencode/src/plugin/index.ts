  ...
  Effect.gen(function* () {
    ...
    const state = yield* InstanceState.make<State>(
      Effect.fn("Plugin.state")(function* (ctx) {
        ...
        yield* PluginDiscovery.logIssues([...extensions.errors, ...extensions.warnings])
        for (const ext of extensions.extensions) {
          ...
          if (ext.extension.entry) {
            ...
            const mod = yield* Effect.tryPromise({
              ...
              Effect.tapError((error) => Effect.logError("failed to load agent plugin", { plugin: ext.name, error })),
              Effect.option,
            )
          ...
          if (shell) {
            yield* PluginDiscovery.logIssues(shell.warnings)
