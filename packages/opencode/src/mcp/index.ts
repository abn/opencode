  ...
  Effect.gen(function* () {
    ...
    const state = yield* InstanceState.make<State>(
      Effect.fn("MCP.state")(function* () {
        ...
        // names. User config wins on collision; plugin servers land in state
        // config so status, connect, and auth flows treat them uniformly.
        ...
        for (const issue of [...discovered.errors, ...discovered.warnings]) {
        ...
        // Discovered plugin servers land in state config so status, connect,
        // auth, and timeout flows treat them like user servers. User servers
        // (cfg.mcp) stay out of s.config: they are read straight from config
        // in those flows, while getMcpConfig checks s.config first, so the
...
export const node = LayerNode.make({
  service: Service,
  layer: layer,
  deps: [CrossSpawnSpawner.node, McpAuth.node, EventV2Bridge.node, Config.node, McpBrowser.node, FSUtil.node, Global.node],
})
