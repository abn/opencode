...
export const loadPluginDir = Effect.fn("Discovery.loadPluginDir")(function* (
  ...
  const raw = yield* fs.readJson(path.join(root, "plugin.json")).pipe(
    Effect.catch(() => Effect.succeed(undefined)),
  )
  if (raw === undefined) {
    errors.push({ path: [], message: `Missing or unreadable plugin.json in ${root}` })
    return { root: realRoot, manifest: undefined, skills: [], mcp: undefined, errors, warnings }
  }
...
function collectSkills(fs: FSUtil.Interface, realRoot: string, manifest: Manifest.Info, warnings: Array<Issue>) {
  return Effect.gen(function* () {
    ...
    if (!(yield* fs.isDir(skillsDir))) return skills
...
function collectSkill(
  ...
  return Effect.gen(function* () {
    const real = yield* fs.realPath(dir).pipe(Effect.catch(() => Effect.succeed(dir)))
...
function readMcpConfig(
  ...
  return Effect.gen(function* () {
    const text = yield* fs.readFileStringSafe(path.join(realRoot, "mcp.json")).pipe(
      Effect.catch(() => Effect.succeed(undefined)),
    )
...
function loadExtension(
  ...
  return Effect.gen(function* () {
    ...
    yield* fs.ensureDir(dataDir).pipe(
      Effect.catch(() =>
        Effect.sync(() => {
    ...
    return { dir: real, entry, options, enabled: true, dataDir }
  })
}
...
function resolveContained(
  ...
  return Effect.gen(function* () {
    const target = path.resolve(realRoot, command)
    const real = yield* fs.realPath(target).pipe(Effect.catch(() => Effect.succeed(undefined)))
    if (real === undefined) {
      errors.push({ path: [key, "command"], message: `MCP server "${key}" skipped: command not found` })
      return undefined
    }
    if (!containsPath(realRoot, real)) {
      errors.push({ path: [key, "command"], message: `MCP server "${key}" skipped: command escapes plugin root` })
      return undefined
    }
    // A bundled command must exist at discovery; bare names resolve through
    // the platform executable search at launch instead.
    if (!(yield* fs.isFile(real))) {
      errors.push({ path: [key, "command"], message: `MCP server "${key}" skipped: command not found` })
      return undefined
    }
    return real
