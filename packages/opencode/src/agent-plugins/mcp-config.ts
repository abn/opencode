...
import { isRecord, isStringArray } from "./manifest"
...
export function parse(input: unknown, manifestSchema: string): Result {
  ...
  if (!isRecord(input))
    return {
      ok: false,
      servers: empty,
      errors: [{ path: [], message: "mcp.json must contain a top-level object" }],
      warnings,
    }
...
function parseServer(name: string, entry: unknown): ServerResult {
  ...
  if (entry["type"] === "streamable-http" || entry["type"] === "sse") return parseRemote(path, entry, entry["type"])
  return {
    ok: false,
    errors: [{ path: [...path, "type"], message: 'Field "type" must be "stdio", "streamable-http", or "sse"' }],
  }
}
...
function parseStdio(path: Array<string>, entry: Record<string, unknown>): ServerResult {
  ...
  if (entry["cwd"] !== undefined) {
    ...
    if (typeof cwd !== "string" || classifyCwd(cwd) === "invalid") {
      errors.push({
        path: [...path, "cwd"],
        message: 'Field "cwd" must be "./"-relative, "${PLUGIN_ROOT}[/...]", or "${PLUGIN_DATA}[/...]"',
      })
      return { ok: false, errors }
    }
    server.cwd = cwd
...
function parseRemote(path: Array<string>, entry: Record<string, unknown>, type: RemoteServer["type"]): ServerResult {
  const errors: Array<Issue> = []
  for (const key of Object.keys(entry)) {
  ...
  if (typeof url !== "string") {
    errors.push({ path: [...path, "url"], message: 'Field "url" must be a string' })
    return { ok: false, errors }
  }
  ...
  const server: RemoteServer = { type, url }
