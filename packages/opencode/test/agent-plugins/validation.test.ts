...
describe("agent-plugins.mcp-config", () => {
  test("rejects unknown top-level fields and version mismatch", () => {
    ...
    expect(McpConfig.parse({ $schema: mcpSchema, mcpServers: {} }, "https://example.com/other.json").ok).toBe(false)
  })
  ...
  test.each([["serve"], ["./bin/serve"], ["python3.11"]])("accepts command %s", (command) => {
    const result = McpConfig.parse(
      { $schema: mcpSchema, mcpServers: { s: { type: "stdio", command } } },
      Manifest.SCHEMA,
    )
    ...
    const result = McpConfig.parse(
      {
        $schema: mcpSchema,
        mcpServers: {
          s: { type: "streamable-http", url: "https://example.com/m", headers: { "X-A": "1", "x-a": "2" } },
        },
      },
      Manifest.SCHEMA,
    )
...
describe("agent-plugins.paths", () => {
  test("expands once without rescanning replacements", () => {
    ...
    expect(Paths.expandPlaceholders("${PLUGIN_ROOT}", { root: "/x${PLUGIN_DATA}", data: "/d" })).toBe(
      "/x${PLUGIN_DATA}",
    )
  })
